package com.example.yaohao.testproject.widget.seekbar;
...
import static com.example.yaohao.testproject.widget.seekbar.SeekBar.INDICATOR_MODE_ALWAYS_SHOW;
...
//    private float maxPositiveValue, minPositiveValue;
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        progressHeight = (int) t.getDimension(R.styleable.RangeSeekBar_rsb_progress_height, DensityUtils.dp2px(getContext(),2));
        ...
        tickMarkTextMargin = (int) t.getDimension(R.styleable.RangeSeekBar_rsb_tick_mark_text_margin, DensityUtils.dp2px(getContext(),7));
        tickMarkTextSize = (int) t.getDimension(R.styleable.RangeSeekBar_rsb_tick_mark_text_size, DensityUtils.dp2px(getContext(),12));
        ...
        if (tickMarkTextArray != null) {
            ...
            for (int i = 0; i < tickMarkTextArray.length; i++) {
                ...
                if (tickMarkMode == TRICK_MARK_MODE_OTHER) {
                    ...
                    if (DensityUtils.compareFloat(num, states[0].value) != -1 && DensityUtils.compareFloat(num, states[1].value) != 1 && seekBarMode == SEEKBAR_MODE_RANGE) {
    ...
    public SeekBarState[] getRangeSeekBarState() {
        ...
        if (tickMarkNumber > 1){
            ...
            if (DensityUtils.compareFloat(leftSB.currPercent, 0f) == 0){
                leftSeekBarState.isMin = true;
            }else if (DensityUtils.compareFloat(leftSB.currPercent, 1f) == 0){
                leftSeekBarState.isMax = true;
            }
        }
        ...
        if (seekBarMode == SEEKBAR_MODE_RANGE) {
            ...
            if (tickMarkNumber > 1){
                ...
                if (DensityUtils.compareFloat(rightSB.currPercent, 0f) == 0){
                    rightSeekBarState.isMin = true;
                }else if (DensityUtils.compareFloat(rightSB.currPercent, 1f) == 0){
                    rightSeekBarState.isMax = true;
                }
            }
        }
